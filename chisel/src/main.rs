...
fn execute_module(context: &ModuleContext, module: &mut Module) -> bool {
    ...
    let ret = match name {
        ...
        "repack" => translate_module(module, &Repack::new()).unwrap_or_else(|e| {
            err_exit(&format!("Error in module {}: {}", name, e.description()))
        }),
        "snip" => translate_module(module, &Snip::new()).unwrap_or_else(|e| {
            err_exit(&format!("Error in module {}: {}", name, e.description()))
        }),
        "dropnames" => translate_module(module, &DropSection::NamesSection).unwrap_or_else(|e| {
            err_exit(&format!("Error in module {}: {}", name, e.description()))
        }),
